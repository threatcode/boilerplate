...
IMAGES="${1:-threatcode-rolling}"
ARCHS="${2:-amd64}"

BASE_IMAGES="threatcode-rolling threatcode-dev threatcode-last-release"
EXTRA_IMAGES="threatcode-experimental threatcode-bleeding-edge"
...
# importance then.
ORDER="threatcode-rolling threatcode-last-release threatcode-bleeding-edge threatcode-experimental threatcode-dev"
for image in $(printf "%s\n" $ORDER | tac); do
    if echo "$IMAGES" | grep -qw $image; then
